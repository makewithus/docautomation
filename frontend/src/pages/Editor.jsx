...
export default function Editor() {
  ...
  const [doc, setDoc]               = useState(null);
  const [content, setContent]       = useState(null);
  const [htmlContent, setHtmlContent] = useState("");   // ★ NEW — drives iframe via srcDoc
  const [loading, setLoading]       = useState(true);
  const [saving, setSaving]         = useState(false);
  const [saved, setSaved]           = useState(false);
  const [dirty, setDirty]           = useState(false);
  ...
  useEffect(() => {
    getDocument(id)
      .then(async (res) => {
        ...
        setHtmlContent(withBaseUrl(res.data.html_content));   // ★ NEW — show what backend has immediately
        ...
        try {
          const res2 = await updateDocument(id, blank);
          // setHtmlContent(res2.data.html_content);  
          //  // ★ changed from setIframeKey
          setHtmlContent(withBaseUrl(res.data.html_content));
        } catch (err) {
          console.error("Failed to reset backend content:", err);
        }
      })
    ...
    recognition.onend = async () => {
      ...
      if (finalText.trim()) {
        ...
        try {
          ...
          setContent(gc);
          const res2 = await updateDocument(id, gc);
          // setHtmlContent(res2.data.html_content);
          //    // ★ changed from setIframeKey
          setHtmlContent(withBaseUrl(res.data.html_content));
          setTranscript("");
  ...
  const handleManualFill = async () => {
    ...
    try {
      ...
      setContent(gc);
      const res2 = await updateDocument(id, gc);
      // setHtmlContent(res2.data.html_content);   // ★ changed from setIframeKey
      setHtmlContent(withBaseUrl(res.data.html_content));
      setTranscript("");
      transcriptRef.current = "";
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // setHtmlContent(res.data.html_content);   // ★ changed from setIframeKey
      setHtmlContent(withBaseUrl(res.data.html_content));
      setDirty(false);
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
  ...
  // ── Auto-save on content change (debounced, no flash) ────────
  useEffect(() => {
    if (!content || !dirty) return;
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await updateDocument(id, content);
        // setHtmlContent(res.data.html_content); 
        //   // ★ changed from setIframeKey — smooth update
        setHtmlContent(withBaseUrl(res.data.html_content));
        setDirty(false);
        setSaved(true);
        setTimeout(() => setSaved(false), 1500);
    ...
    }, 500);   // ★ reduced debounce for snappier live feel
  ...
  const handleRefill = async () => {
    ...
    try {
      ...
      setContent(generatedContent);
      const res2 = await updateDocument(id, generatedContent);
      // setHtmlContent(res2.data.html_content);   // ★ changed from setIframeKey
      setHtmlContent(withBaseUrl(res.data.html_content));
      setDirty(false);
      setPrompt("");
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.panels}>
        ...
        {/* Right — live preview — ★ srcDoc instead of src, no more key remount */}
        <div style={s.rightPanel}>
          <div style={s.iframeWrap}>
            <iframe
              srcDoc={htmlContent}
...
function ReceiptFields({ content, update }) {
  ...
  useEffect(() => {
    ...
    if (content.line_items && content.line_items.length > 0) {
      ...
      // line_items aur amount_received dono khaali hain —
      // jo user ne manually Subtotal field mein type kiya, wahi rakho
