...
// // import {refillDocument } from "../services/api";
...
// // REMOVED: refillDocument import — handleRefill belongs in Editor.jsx, not Home.jsx

// // FIXED: "Timeline" → "timeline" (was uppercase, broke type matching everywhere)
// // FIXED: Added icon field to each type (was referenced in JSX but missing from objects)
// const DOC_TYPES = [
//   {
//     id:       "timeline",
//     label:    "Timeline",
//     icon:     "📅",
//     desc:     "Project phases, milestones & sprint plan",
//     keywords: ["deadline", "status", "deployment_phase", "testing_phase"],
//   },
//   {
//     id:       "client_doc",
...
//   // REMOVED: prompt, generating, promptOpen states — these belong in Editor.jsx only
//   // REMOVED: handleRefill — used id/content/setContent which don't exist in Home
...
//         navigate(responseData.edit_url);
//       } else {
//         console.error("No doc_id or edit_url found", responseData);
//         setError("Document URL not found in response.");
//         setStep(2);
//       }
//     } catch (e) {
//       clearInterval(interval);
...
//                 {DOC_TYPES.map(t => (
//                   <div key={t.id} style={s.typeCard}>
//                     {/* FIXED: t.icon now exists — was undefined before */}
//                     <span style={s.typeIcon}>{t.icon}</span>
//                     <div style={s.typeLabel}>{t.label}</div>
//                     <div style={s.typeDesc}>{t.desc}</div>
//                   </div>
...
//                 </div>
//               </div>
//               <button style={s.changeBtn} onClick={handleReset}>✕ Change file</button>
//             </div>

//             {detectedDocType && (
//               <div style={s.detectedBox}>
//                 <div style={s.detectedLeft}>
//                   {/* FIXED: detectedDocType.icon now exists */}
//                   <span style={s.detectedIcon}>{detectedDocType.icon}</span>
//                   <div>
//                     <div style={s.detectedLabel}>AI detected document type</div>
//                     <div style={s.detectedType}>{detectedDocType.label}</div>
...
//                   src={logo}
//                   alt="Logo"
//                   style={{ width: "60px", height: "60px", objectFit: "contain", animation: "spin 2s linear infinite" }}
//                 />
//               </div>
...
// // ── Styles ─────────────────────────────────────────────────────────────────
// const s = {
//   // FIXED: s.page was defined twice — first definition was silently overwritten.
//   // Kept the more complete second definition (flex column, overflow hidden).
//   page:        { minHeight: "100vh", background: "#fafafa", fontFamily: "system-ui, -apple-system, sans-serif" },
//   wrap:        { maxWidth: 680, margin: "0 auto", padding: "40px 24px 100px" },
...
//   // FIXED: s.spinner was defined twice — removed duplicate, kept one clean definition
//   spinner:     { width: 28, height: 28, border: "2.5px solid #eee", borderTopColor: "#111", borderRadius: "50%", animation: "spin 0.8s linear infinite" },
...
//   dropzone:       { border: "1.5px dashed #d0d0d0", borderRadius: 14, padding: "56px 24px", cursor: "pointer", background: "#fff", marginBottom: 16, transition: "border-color .2s, background .2s", userSelect: "none" },
//   dropzoneActive: { borderColor: "#888", background: "#f8f8f8", cursor: "wait" },
//   dzInner:     { display: "flex", flexDirection: "column", alignItems: "center", gap: 10 },
//   dzIconWrap:  { width: 52, height: 52, background: "#f5f5f5", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" },
//   dzIcon:      { fontSize: 24 },
//   dzLabel:     { fontSize: 15, fontWeight: 600, color: "#111" },
...
//   error:       { fontSize: 13, color: "#c0392b", background: "#fdf2f2", border: "1px solid #f5c6cb", borderRadius: 8, padding: "10px 14px", marginBottom: 16, lineHeight: 1.5 },
...
//   confirmWrap:     { display: "flex", flexDirection: "column", gap: 16 },
//   confirmFile:     { display: "flex", alignItems: "center", gap: 14, background: "#fff", border: "1px solid #efefef", borderRadius: 12, padding: "16px 18px" },
//   confirmFileIcon: { fontSize: 28, flexShrink: 0 },
//   confirmFileInfo: { flex: 1 },
//   confirmFileName: { fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 3 },
//   confirmFileMeta: { fontSize: 12, color: "#999" },
//   changeBtn:       { fontSize: 12, color: "#666", background: "none", border: "1px solid #e0e0e0", borderRadius: 6, padding: "6px 12px", cursor: "pointer", flexShrink: 0 },

//   detectedBox:   { display: "flex", alignItems: "center", justifyContent: "space-between", background: "#f0f7ff", border: "1px solid #cce0ff", borderRadius: 12, padding: "14px 18px" },
//   detectedLeft:  { display: "flex", alignItems: "flex-start", gap: 12 },
//   detectedIcon:  { fontSize: 24, flexShrink: 0 },
//   detectedLabel: { fontSize: 11, color: "#666", marginBottom: 3 },
//   detectedType:  { fontSize: 14, fontWeight: 700, color: "#1a56db", marginBottom: 2 },
//   detectedDesc:  { fontSize: 12, color: "#888" },
//   detectedDot:   { width: 10, height: 10, background: "#27ae60", borderRadius: "50%", flexShrink: 0 },

//   previewBox:   { background: "#fff", border: "1px solid #efefef", borderRadius: 10, padding: "14px 16px" },
//   previewLabel: { fontSize: 11, fontWeight: 600, color: "#aaa", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 },
//   previewText:  { fontSize: 12, color: "#666", lineHeight: 1.6, fontFamily: "monospace", whiteSpace: "pre-wrap", maxHeight: 160, overflow: "hidden" },

//   confirmActions: { display: "flex", gap: 10, flexWrap: "wrap" },
//   btnGenerate:    { fontSize: 14, fontWeight: 600, color: "#fff", background: "#111", border: "none", borderRadius: 8, padding: "12px 28px", cursor: "pointer" },
//   btnSecondary:   { fontSize: 13, color: "#666", background: "none", border: "1px solid #ddd", borderRadius: 8, padding: "12px 18px", cursor: "pointer" },

//   generatingWrap:  { display: "flex", justifyContent: "center", alignItems: "center", minHeight: 420 },
//   generatingInner: { textAlign: "center", maxWidth: 380 },
//   genLogoWrap:     { marginBottom: 24, display: "flex", justifyContent: "center" },
//   genTitle:        { fontSize: 22, fontWeight: 700, color: "#111", marginBottom: 8, letterSpacing: -0.5 },
//   genSub:          { fontSize: 14, color: "#999", marginBottom: 36 },
//   genSteps:        { textAlign: "left", display: "inline-block" },
//   genStepRow:      { display: "flex", alignItems: "center", gap: 12, padding: "6px 0", transition: "opacity .4s" },
//   genStepDot:      { width: 20, height: 20, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .3s" },
//   genActiveDot:    { width: 8, height: 8, borderRadius: "50%", background: "#111" },
//   genStepLabel:    { fontSize: 14, transition: "all .3s" },
// };
...
export default function Home() {
  const navigate = useNavigate();
  const [creating, setCreating] = useState(null); // which template is loading

  const handleSelect = async (templateType) => {
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        {/* <a href="/documents" style={s.allDocs}>View all documents →</a> */}
      </div>
      ...
      <div style={s.grid}>
        {TEMPLATES.map((t) => (
          <button
            ...
            style={{
              ...s.card,
              opacity: creating && creating !== t.type ? 0.5 : 1,
            }}
          ...
          >
            ...
            {creating === t.type ? (
              ...
              <>
                <div style={{ ...s.cardIcon, background: t.color, color: t.accent }}>
                  {t.icon}
                </div>
                <div style={s.cardTitle}>{t.title}</div>
                <div style={s.cardDesc}>{t.description}</div>
                <div style={{ ...s.cardTag, background: t.color, color: t.accent }}>
                  Use template →
                </div>
              </>
            )}
...
const s = {
  page:      { minHeight: "100vh", background: "#f7f7f7", fontFamily: "system-ui,-apple-system,sans-serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 24px 60px" },
  header:    { width: "100%", maxWidth: 860, display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 0" },
  logo:      { display: "flex", alignItems: "center", gap: 8 },
  logoText:  { fontSize: 16, fontWeight: 700, color: "#111", letterSpacing: "-0.3px" },
  allDocs:   { fontSize: 25, color: "#888", textDecoration: "none" },
  ...
  cardLoading: { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, minHeight: 120 },
  spinner:     { width: 24, height: 24, border: "2.5px solid #eee", borderTopColor: "#111", borderRadius: "50%", animation: "spin .8s linear infinite" },
  // cardIcon:    { width: 44, height: 44, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 },
  cardTitle:   { fontSize: 19, fontWeight: 700, color: "#111" },
  cardDesc:    { fontSize: 15, color: "#888", lineHeight: 1.5 },
  cardTag:     { alignSelf: "flex-start", fontSize: 15, fontWeight: 600, padding: "3px 10px", borderRadius: 20, marginTop: 4 },
