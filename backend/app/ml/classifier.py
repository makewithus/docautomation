...
receipt_template:
- A PAYMENT RECEIPT acknowledging payment received from a client
- Includes Receipt No, Date, Received From, Payment Mode, Service, Amount Received, Balance After Payment, Payment Status, and Paid On
- May include Amount in Words, Bank Details, GST, and an acknowledgement note
- Used as proof of payment after receiving an installment or project payment
- Example keywords: receipt no, received from, amount received, payment receipt, payment mode, balance after payment, paid on, receipt, acknowledgement
...
async def classify_document_ai(text: str) -> str:
    ...
    try:
        ...
        valid = {"receipt_template", "client_doc", "compliance", "invoice", "timeline"}
        if result in valid:
            return result
...
def _keyword_classify(text: str) -> str:
    ...
    scores = {
        "receipt_template": sum(1 for kw in [
            "requirement", "feature", "module", "user story", "scope",
            "backend", "frontend", "api", "mvp", "onboarding", "development",
