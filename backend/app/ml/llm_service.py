...
2. SYSTEM_PROMPT now instructs AI to write full document content from NL input
   → body_paragraphs, descriptions, letters are fully AI-written, not placeholders
3. Schemas cleaned — removed pre-filled amount/subtotal/gst/total fields from
   client_doc, receipt_template and invoice (template_engine.py owns all math now)
"""
...
- The user's input may be in Malayalam, Hindi, Tamil, Telugu, Kannada, or any other language.
- First understand the meaning and intent of the user's input.
- ALWAYS generate the final document in professional English.
- Never generate the output in the user's input language.
- Keep names, company names, phone numbers, email addresses, dates, invoice numbers, IDs, and currency values unchanged.
...
- The user's input may be in Malayalam, Hindi, Tamil, Telugu, Kannada, or any other language.
- First understand the meaning of the input.
- ALWAYS generate the final document in professional English.
- Never generate the document in the input language.
- Keep names, phone numbers, IDs, dates, email addresses and currency values unchanged.
...
"""
    ...
    def generate_raw(self, prompt: str) -> dict:
        """Send raw prompt, parse JSON response."""
